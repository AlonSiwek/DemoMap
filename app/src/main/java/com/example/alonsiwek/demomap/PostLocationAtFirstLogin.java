...
public class PostLocationAtFirstLogin extends AsyncTask<JSONObject,String,String> {
    ...
    private String postData(String urlPath) throws IOException , JSONException{
        ...
        // get the current location for first Login
        LocationManager locationManager = (LocationManager) ctx.getSystemService(LOCATION_SERVICE);
        Location currentLocation = locationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER);

        if (currentLocation != null) {
            Log.d("PostLocation", "the location is: " + currentLocation);
        } else {
            Log.d("PostLocation", "location is : NULL");
        }
        ...
        try {
            ...
            firstPostOnDB.put("loc",inside_loc);
            firstPostOnDB.put("user_name", data_name);
            firstPostOnDB.put("phone_number", data_phone);
